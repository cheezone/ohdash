...
describe("last", () => {
  it("获取数组中的最后一个水果", () => {
    ...
    expect(最后一个(fruits)).toBe("橙子"); // 测试别名
  ...
  it("获取空数组中的最后一个元素，返回 undefined", () => {
    ...
    expect(最后一个(emptyArray)).toBe(undefined);
  ...
  it("类型测试", () => {
    ...
    expectTypeOf(最后一个(fruits)).toEqualTypeOf<"苹果" | "香蕉" | "橙子">();

    const emptyArray = [] as [];
    expectTypeOf(last(emptyArray)).toBeUndefined();
    expectTypeOf(最后一个(emptyArray)).toBeUndefined();
  });
});
